...
for color in colors
    for char in alphanumeric
        for i in 1:3
            ...
            Drawing((55/60)*2*x, (57/60)*2*x, "../shapes/pentagons/$(color)_$(char)_$(i).png")
            origin()
            sethue(rgb_color)
            ngon(-(1/10)*x, 0, x*0.98, 5, :fill)
            sethue(text_color)
            setfont("Arial", (4/3)*x)
            settext(char, Point(-(2/10)*x, 0), halign="center", valign="center", angle=-90)
            finish()
        end
    end
end